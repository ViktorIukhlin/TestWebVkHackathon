...
const Basket = ({ match: { params: { areaId, itemId }}, foodAreas, order }) => {
  const [ faster, setFaster ] = useReducer((state, updatedState) => {
    localStorage.setItem("faster", updatedState);

    return updatedState;
  }, !localStorage.getItem("time"));

  const [ time, setTime ] = useReducer((state, updatedState) => {
    localStorage.setItem("time", updatedState);

    return updatedState;
  }, localStorage.getItem("time") || "");

  const [ selfService, setSelfService ] = useReducer((state, updatedState) => {
    localStorage.setItem("selfService", updatedState);

    return updatedState;
  }, localStorage.getItem("selfService") || false);
